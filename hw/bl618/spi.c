/*
 * spi.c - TX-only SPI0 driver for BL618
 *
...
#define	SPI_BASE (mmio_m0_base + 0xa200)
...
#define	SPI_BUS_BUSY \
	(*(volatile uint32_t *) (SPI_BASE + 0x8))
#define	SPI_PRD0 \
...
#define	SPI_CFG0 \
	(*(volatile uint32_t *) (SPI_BASE + 0x80))
...
#define	SPI_CFG1 \
	(*(volatile uint32_t *) (SPI_BASE + 0x84))
...
#define	SPI_WDATA \
	(*(volatile uint32_t *) (SPI_BASE + 0x88))
#define	SPI_RDATA(spi) \
	(*(volatile uint32_t *) (SPI_BASE + 0x8c))
...
{
	while (len--) {
		while (!SPI_GET(CFG1_TX_FIFO_CNT, SPI_CFG1));
		SPI_WDATA = *(uint8_t *) data++;
	}
}
...
{
	SPI_CFG = SPI_MASK_CFG_SCLK_PH | SPI_MASK_CFG_M_EN;
